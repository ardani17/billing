"use client";

import { useState } from "react";
import type { ExpenseCategory } from "../../reports/lib/types";
import { createCategory, updateCategory, deleteCategory } from "../../reports/lib/api";

interface CategoryManagerProps {
  categories: ExpenseCategory[];
  onUpdate: () => void;
}

export function CategoryManager({ categories, onUpdate }: CategoryManagerProps) {
  const [newName, setNewName] = useState("");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editName, setEditName] = useState("");
  const [error, setError] = useState<string | null>(null);

  const handleCreate = async () => {
    if (!newName.trim()) return;
    setError(null);
    try {
      await createCategory({ name: newName.trim() });
      setNewName("");
      onUpdate();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Gagal menambah kategori");
    }
  };

  const handleUpdate = async (id: string) => {
    if (!editName.trim()) return;
    setError(null);
    try {
      await updateCategory(id, { name: editName.trim() });
      setEditingId(null);
      onUpdate();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Gagal mengubah kategori");
    }
  };

  const handleDelete = async (id: string) => {
    setError(null);
    try {
      await deleteCategory(id);
      onUpdate();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Gagal menghapus kategori. Pastikan tidak ada pengeluaran yang menggunakan kategori ini.");
    }
  };

  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <h3 className="mb-4 text-sm font-semibold text-slate-900">Kelola Kategori</h3>

      {/* Tambah kategori*/}
      <div className="mb-4 flex gap-2">
        <input
          type="text"
          value={newName}
          onChange={(e) => setNewName(e.target.value)}
          placeholder="Nama kategori baru"
          className="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
          style={{ minHeight: 44 }}
          onKeyDown={(e) => e.key === "Enter" && handleCreate()}
        />
        <button
          type="button"
          onClick={handleCreate}
          className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"
          style={{ minHeight: 44 }}
        >
          Tambah
        </button>
      </div>

      {error && (
        <div className="mb-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>
      )}

      {/* Daftar kategori*/}
      <div className="space-y-2">
        {categories.map((c) => (
          <div key={c.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-4 py-2">
            {editingId === c.id ? (
              <div className="flex flex-1 items-center gap-2">
                <input
                  type="text"
                  value={editName}
                  onChange={(e) => setEditName(e.target.value)}
                  className="flex-1 rounded-lg border border-slate-300 px-3 py-1 text-sm focus:border-blue-500 focus:outline-none"
                  style={{ minHeight: 44 }}
                  onKeyDown={(e) => e.key === "Enter" && handleUpdate(c.id)}
                />
                <button
                  type="button"
                  onClick={() => handleUpdate(c.id)}
                  className="rounded-lg bg-blue-600 px-3 py-1 text-xs font-medium text-white hover:bg-blue-700"
                  style={{ minHeight: 44 }}
                >
                  Simpan
                </button>
                <button
                  type="button"
                  onClick={() => setEditingId(null)}
                  className="rounded-lg px-3 py-1 text-xs text-slate-500 hover:bg-slate-100"
                  style={{ minHeight: 44 }}
                >
                  Batal
                </button>
              </div>
            ) : (
              <>
                <div className="flex items-center gap-2">
                  <span className="text-sm text-slate-700">{c.name}</span>
                  {c.is_default && (
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-500">Default</span>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <button
                    type="button"
                    onClick={() => { setEditingId(c.id); setEditName(c.name); }}
                    className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600"
                    style={{ minHeight: 44, minWidth: 44 }}
                    aria-label="Edit kategori"
                  >
                    <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>
                  </button>
                  {!c.is_default && (
                    <button
                      type="button"
                      onClick={() => handleDelete(c.id)}
                      className="rounded-lg p-2 text-red-400 hover:bg-red-50 hover:text-red-600"
                      style={{ minHeight: 44, minWidth: 44 }}
                      aria-label="Hapus kategori"
                    >
                      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                      </svg>
                    </button>
                  )}
                </div>
              </>
            )}
          </div>
        ))}
      </div>
    </div>
  );
}
