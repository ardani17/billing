// gateway_link_test.go berisi unit test untuk GatewayUsecase — manajemen payment link.
...
func TestGeneratePaymentLink_SkipsLunasBatal(t *testing.T) {
	...
	// Error terjadi sebelum adapter dipanggil
	if errors.Is(err, domain.ErrGatewayUnavailable) {
...
func TestRegeneratePaymentLink_ExpiresOldAndCreatesNew(t *testing.T) {
	...
	// Generate baru gagal di adapter
	if !errors.Is(err, domain.ErrGatewayUnavailable) {
		t.Fatalf("expected ErrGatewayUnavailable, got %v", err)
	}
...
// TestWalledGardenPaymentInfo_NoActiveLink — generate on-demand gagal, info tetap ada.
func TestWalledGardenPaymentInfo_NoActiveLink(t *testing.T) {
	...
	// PaymentURL kosong karena generate gagal
	if info.PaymentURL != "" {
...
func TestSyncPaymentLinkAmount_ExpiresAndRegenerates(t *testing.T) {
	...
	// Generate baru gagal di adapter, tapi link lama harus di-expire
