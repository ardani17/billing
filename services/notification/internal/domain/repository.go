...
// Diimplementasikan oleh repositori.ConfigRepo.
type ConfigRepository interface {
	...
	// Upsert membuat atau memperbarui konfigurasi notifikasi (insert jika belum ada, perbarui jika sudah).
...
// Diimplementasikan oleh repositori.TemplateRepo.
type TemplateRepository interface {
	...
	// Perbarui memperbarui template notifikasi dan mengembalikan template yang diperbarui.
	...
	// Hanya template kustom (is_default=false) yang boleh dihapus.
	...
	// BulkCreate membuat beberapa template sekaligus (digunakan untuk seeding bawaan templates).
	BulkCreate(ctx context.Context, templates []*NotificationTemplate) error

	// SlugExists mengecek apakah slug sudah ada di tenant (exclude ID tertentu untuk perbarui).
...
// Diimplementasikan oleh repositori.LogRepo.
type LogRepository interface {
	...
	// Perbarui memperbarui catatan log notifikasi (status, retry_count, error_message, sent_at).
