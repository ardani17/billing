...
// Compile-time check: vlanManager harus mengimplementasikan domain.VLANManager.
...
// Create membuat VLAN baru untuk OLT tertentu.
...
// Update memperbarui data VLAN yang sudah ada.
...
// Delete melakukan soft-delete VLAN setelah memastikan tidak ada ONT aktif yang menggunakan.
...
// resolveSingle mengembalikan VLAN default untuk OLT (strategy "single").
...
func (vm *vlanManager) resolvePerPaket(ctx context.Context, oltID, packageID string) (*domain.VLAN, error) {
	...
	// Fallback ke default VLAN jika tidak ada mapping spesifik
	return vm.resolveSingle(ctx, oltID)
}
...
func (vm *vlanManager) resolvePerPelanggan(ctx context.Context, oltID, customerID string) (*domain.VLAN, error) {
	...
	// Untuk per_pelanggan, tidak ada fallback — harus ada VLAN unik
	return nil, domain.ErrVLANResolutionFailed
}
