...
// File ini mengimplementasikan OLTHealthChecker untuk monitoring periodik OLT via SNMP Ping.
package usecase
...
// Compile-time check: oltHealthChecker harus mengimplementasikan domain.OLTHealthChecker.
var _ domain.OLTHealthChecker = (*oltHealthChecker)(nil)

// oltWorker menyimpan state goroutine health check untuk satu OLT.
...
// Menjalankan satu goroutine ticker per OLT untuk monitoring periodik via SNMP Ping.
...
// Stop menghentikan semua goroutine health check OLT.
...
// RemoveOLT menghapus OLT dari health check schedule dan menghentikan goroutine-nya.
...
func (hc *oltHealthChecker) UpdateInterval(oltID string, intervalSec int) {
	...
	// Salin data OLT dan update interval
	oltCopy := *w.olt
...
// runWorker menjalankan loop health check untuk satu OLT dengan ticker.
func (hc *oltHealthChecker) runWorker(ctx context.Context, olt *domain.OLT) {
	...
	if interval <= 0 {
		interval = 300 * time.Second // default 5 menit
	}
