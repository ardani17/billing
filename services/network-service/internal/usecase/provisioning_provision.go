...
// Validasi input, resolve profile & VLAN, eksekusi CLI command, update DB, audit log, publish event.
package usecase
...
// Flow: validate → resolve profile & VLAN → create ONT → execute CLI → update status → audit → event.
func (pm *provisioningManager) ProvisionONT(ctx context.Context, tenantID string, req domain.ProvisionONTRequest) (*domain.ONTResponse, error) {
	...
	// Update status ke provisioned + completed
