...
// Langkah: validasi router online → ambil pool → build commands → execute → update status.
func (m *vpnManager) AutoConfigure(ctx context.Context, id string) error {
	...
	// Build dan execute commands sesuai protokol
	if err := m.executeVPNCommands(ctx, pool, tunnel); err != nil {
		m.logger.Error().Err(err).Str("tunnel_id", id).Msg("auto-configure vpn gagal")
		return domain.ErrAutoConfigFailed
	}

	// Update status tunnel ke pending (menunggu verifikasi)
...
func (m *vpnManager) executeVPNCommands(ctx context.Context, pool domain.ConnPool, tunnel *domain.VPNTunnel) error {
	...
	commands, err := m.buildVPNCommands(tunnel)
	if err != nil {
		return err
	}
...
func (m *vpnManager) buildVPNCommands(tunnel *domain.VPNTunnel) ([]vpnCommand, error) {
	var cmds []vpnCommand
	interfaceName := fmt.Sprintf("ispboss-vpn-%s", tunnel.TunnelName)

	switch tunnel.Protocol {
	case domain.ProtocolWireGuard:
		cmds = m.buildWireGuardCommands(tunnel, interfaceName)
	case domain.ProtocolL2TPIPSec:
		cmds = m.buildL2TPCommands(tunnel, interfaceName)
	case domain.ProtocolPPTP:
		cmds = m.buildPPTPCommands(tunnel, interfaceName)
	case domain.ProtocolSSTP:
		cmds = m.buildSSTPCommands(tunnel, interfaceName)
	case domain.ProtocolOpenVPN:
		cmds = m.buildOpenVPNCommands(tunnel, interfaceName)
	default:
	...
	cmds = append(cmds, m.buildCommonCommands(tunnel, interfaceName)...)

	return cmds, nil
}

// buildWireGuardCommands membangun perintah WireGuard.
func (m *vpnManager) buildWireGuardCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	cmd1Path, cmd1Args := m.cmdBuilder.CreateWireGuardInterface(domain.WireGuardInterfaceParams{
		Name: ifName, ListenPort: t.ListenPort, PrivateKey: t.ClientPrivateKeyEncrypted,
	})
	endpoint := extractHost(t.ServerEndpoint)
	cmd2Path, cmd2Args := m.cmdBuilder.AddWireGuardPeer(domain.WireGuardPeerParams{
...
func (m *vpnManager) buildL2TPCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	endpoint := extractHost(t.ServerEndpoint)
	path, args := m.cmdBuilder.CreateL2TPClient(domain.L2TPClientParams{
...
func (m *vpnManager) buildPPTPCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	endpoint := extractHost(t.ServerEndpoint)
	path, args := m.cmdBuilder.CreatePPTPClient(domain.PPTPClientParams{
		Name: ifName, ConnectTo: endpoint, User: t.L2TPUsername,
...
func (m *vpnManager) buildSSTPCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	endpoint := extractHost(t.ServerEndpoint)
	path, args := m.cmdBuilder.CreateSSTPClient(domain.SSTPClientParams{
		Name: ifName, ConnectTo: endpoint, User: t.L2TPUsername,
		Password: t.L2TPPasswordEncrypted, Profile: "default-encryption",
...
func (m *vpnManager) buildOpenVPNCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	endpoint := extractHost(t.ServerEndpoint)
	path, args := m.cmdBuilder.CreateOpenVPNClient(domain.OpenVPNClientParams{
...
func (m *vpnManager) buildCommonCommands(t *domain.VPNTunnel, ifName string) []vpnCommand {
	ipPath, ipArgs := m.cmdBuilder.AddIPAddress(domain.IPAddressParams{
		Address: t.VPNIP + "/24", Interface: ifName,
		Comment: fmt.Sprintf("ISPBoss:vpn:%s", t.ID),
	})
	routePath, routeArgs := m.cmdBuilder.AddIPRoute(domain.IPRouteParams{
