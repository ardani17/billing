// Package usecase — Property test untuk bulk provisioning count invariant + unit tests.
// Memverifikasi bahwa success_count + failure_count == total valid rows.
package usecase
...
//
// Untuk sembarang CSV input dengan N baris valid,
// setelah ExecuteBulk, success_count + failure_count == N.
...
// success_count + failure_count == jumlah baris valid dari preview.
//
// **Validates: Requirements 5.2, 5.6**
func TestProperty2_BulkCountInvariant(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		// Generate 1-10 baris CSV
		...
		// Buat manager dengan mock dependencies
		...
		// INVARIANT: success + failure == total valid rows
...
// TestValidateBulk_ValidCSV memverifikasi parsing CSV yang valid.
...
// TestValidateBulk_InvalidCSV memverifikasi error saat CSV format tidak valid.
...
// TestExecuteBulk_NotFound memverifikasi error saat bulk_id tidak ditemukan.
