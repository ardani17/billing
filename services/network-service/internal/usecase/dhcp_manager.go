...
func (m *dhcpManager) DeleteBinding(ctx context.Context, routerID, bindingID string, req domain.DeleteDHCPBindingRequest) error {
	...
	if leaseID, _ := findDHCPLeaseID(ctx, adapter, binding); leaseID != "" {
		if _, err := adapter.Execute(ctx, "/ip/dhcp-server/lease/remove", map[string]string{"=numbers": leaseID}); err != nil {
			m.audit(ctx, binding, "dhcp.binding.delete", "/ip/dhcp-server/lease/remove", "failed", err)
			return err
		}
	}
...
func (m *dhcpManager) syncBindingToRouter(ctx context.Context, binding *domain.DHCPBinding) error {
	...
	if leaseID != "" {
		...
		if _, err := adapter.Execute(ctx, "/ip/dhcp-server/lease/set", params); err != nil {
			return err
		}
	} else {
		if _, err := adapter.Execute(ctx, "/ip/dhcp-server/lease/add", params); err != nil {
			return err
		}
