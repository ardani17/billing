// Package usecase — Property test untuk default provisioning settings.
// Memverifikasi bahwa tenant tanpa settings record mendapat default values
...
// di database, DefaultProvisioningSettings SHALL mengembalikan:
...
// nilai default yang benar dan konsisten.
//
// **Validates: Requirements 15.5**
...
// nilai default yang identik (kecuali timestamp).
//
// **Validates: Requirements 15.5**
func TestProperty8_DefaultSettings_Consistency(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		// Kedua panggilan harus menghasilkan default values yang sama
...
// untuk tenant_id yang berbeda, default values tetap sama
...
func TestProperty8_DefaultSettings_DifferentTenants(t *testing.T) {
	rapid.Check(t, func(rt *rapid.T) {
		...
		// Default values harus sama untuk semua tenant
