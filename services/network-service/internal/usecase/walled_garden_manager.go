...
func (m *walledGardenManager) Remove(ctx context.Context, routerID string) (*domain.WalledGardenStatus, error) {
	...
	if err := removeWalledGardenManaged(ctx, adapter); err != nil {
...
func ensureAllowedDestinations(ctx context.Context, adapter domain.RouterOSAdapter, cfg domain.WalledGardenConfig) error {
...
func ensureWalledGardenRules(ctx context.Context, adapter domain.RouterOSAdapter, cfg domain.WalledGardenConfig) error {
	rules := rulesForWalledGarden(cfg)
	for _, rule := range rules {
		if err := upsertFirewallRule(ctx, adapter, rule); err != nil {
			return err
		}
	}
	return nil
...
func pruneWalledGardenRules(ctx context.Context, adapter domain.RouterOSAdapter, cfg domain.WalledGardenConfig) error {
	...
	for _, target := range []string{"/ip/firewall/nat", "/ip/firewall/filter"} {
		...
		for _, row := range rows {
			...
			if strings.HasPrefix(comment, walledGardenComment) && !keep[comment] {
				if _, err := adapter.Execute(ctx, target+"/remove", map[string]string{"=numbers": row[".id"]}); err != nil {
					return err
				}
			}
		}
...
func upsertFirewallRule(ctx context.Context, adapter domain.RouterOSAdapter, spec firewallRuleSpec) error {
	...
	if id := findCommentID(rows, spec.Comment); id != "" {
		params["=numbers"] = id
		if _, err = adapter.Execute(ctx, commandPrefix+"/set", params); err != nil {
			return err
		}
	} else if _, err = adapter.Execute(ctx, commandPrefix+"/add", params); err != nil {
		return err
	}
...
func removeWalledGardenManaged(ctx context.Context, adapter domain.RouterOSAdapter) error {
	for _, target := range []struct {
		...
		for _, row := range rows {
			...
			if _, err := adapter.Execute(ctx, target.command+"/remove", map[string]string{"=numbers": row[".id"]}); err != nil {
				return err
			}
		}
	}
