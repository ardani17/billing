...
// File ini mengimplementasikan VPNBandwidthStore menggunakan Redis sorted sets.
...
// vpnBandwidthStore mengimplementasikan domain.VPNBandwidthStore menggunakan Redis sorted sets.
// Setiap tunnel memiliki sorted set dengan key "vpn:bw:{tunnel_id}",
...
// bwKey mengembalikan key Redis sorted set untuk tunnel tertentu.
...
// Query mengambil data point bandwidth dalam rentang waktu [from, to].
// Mengembalikan slice VPNBandwidthPoint yang sudah sorted ascending berdasarkan timestamp.
func (s *vpnBandwidthStore) Query(ctx context.Context, tunnelID string, from, to time.Time) ([]domain.VPNBandwidthPoint, error) {
	...
	// Parse setiap member menjadi VPNBandwidthPoint
...
// parseBWMember mengekstrak VPNBandwidthPoint dari member sorted set.
