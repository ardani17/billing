...
// =============================================================================
// Feature: mikrotik-pppoe, Property 7: Retry backoff schedule
// =============================================================================

// TestProperty_RetryBackoffSchedule verifies that for any retry attempt number N
// where 0 ≤ N < 5, the calculated backoff delay matches the expected schedule:
// attempt 0 → 30s, attempt 1 → 60s, attempt 2 → 120s, attempt 3 → 300s, attempt 4 → 600s.
// For N ≥ 5, the delay should be the last value (600s).
//
...
func TestProperty_RetryBackoffSchedule(t *testing.T) {
	...
	t.Run("beyond_range", func(t *testing.T) {
		rapid.Check(t, func(t *rapid.T) {
			// Generate attempt numbers >= 5 (up to a reasonable upper bound)
...
// contains exactly 5 entries with the correct values.
...
// TestPPPoERetryDelay_EachAttempt verifies PPPoERetryDelay returns the correct
// delay for each attempt 0 through 4.
...
// TestPPPoERetryDelay_BeyondMaxRetries verifies that for attempt >= 5,
// PPPoERetryDelay returns the last delay value (600s).
...
// TestEventTypeConstants verifies that all event type constants are defined
...
// TestMaxRetries verifies the maxRetries constant is set to 5.
