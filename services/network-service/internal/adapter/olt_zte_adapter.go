...
// Compile-time check: pastikan ZTEAdapter mengimplementasikan domain.OLTAdapter.
var _ domain.OLTAdapter = (*ZTEAdapter)(nil)

// ZTEAdapter mengimplementasikan domain.OLTAdapter untuk brand ZTE.
// Menggunakan SNMP untuk monitoring dan CLI untuk provisioning.
...
// Parse brand/model/firmware dari sysDescr string.
...
func (a *ZTEAdapter) GetSFPInfo(ctx context.Context, portIndex int) (*domain.SFPInfo, error) {
	oltIdx := zteCalculateOLTIndex(0, portIndex)
	oids := []string{
