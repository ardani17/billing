...
// Default timeout untuk koneksi CLI.
const (
...
// applyDefaults mengisi nilai default pada CLIConfig jika belum diset.
...
func (c *cliConnector) executeSSH(ctx context.Context, cfg domain.CLIConfig, command string) (string, error) {
	...
	case err := <-done:
		if err != nil {
			...
			// OLT CLI kadang return exit code 1 tapi output tetap valid
			if output.Len() > 0 {
