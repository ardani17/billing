// olt_handler.go menangani HTTP request untuk manajemen OLT device.
// Termasuk: CRUD, test SNMP/CLI, dan status summary.
// Endpoint monitoring (PON ports, ONT, traffic, alarm, SFP, capacity) ada di olt_handler_monitoring.go.
package handler
...
// OLTHandler menangani HTTP request untuk operasi OLT device.
...
// Parse body, validasi, extract tenant_id, lalu buat OLT baru.
...
// Parse query params (page, page_size, status, brand, search) dan return paginated list.
...
// Parse body, validasi, lalu update data OLT.
...
// Soft-delete OLT dan stop health check monitoring.
