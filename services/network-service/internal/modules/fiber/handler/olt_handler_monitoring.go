// olt_handler_monitoring.go menangani HTTP permintaan pemantauan OLT.
...
func (h *OLTHandler) GetTraffic(c *fiber.Ctx) error {
	...
	if fromStr := c.Query("from"); fromStr != "" {
		...
		from = time.Now().Add(-1 * time.Hour) // bawaan 1 jam terakhir
	}
	...
	data, err := h.oltManager.GetTraffic(c.UserContext(), id, portIndex, from, to)
	if err != nil {
		return h.mapError(c, err)
	}
...
// GetSignal menangani GET /devices/:id/pon-ports/:port/onts/:ont/signal.
func (h *OLTHandler) GetSignal(c *fiber.Ctx) error {
	id := c.Params("id")
	if id == "" {
		return domain.ErrorResponse(c, fiber.StatusBadRequest, "BAD_REQUEST", "OLT ID wajib diisi")
	}
	...
	ontIndex, err := strconv.Atoi(c.Params("ont"))
	if err != nil {
		return domain.ErrorResponse(c, fiber.StatusBadRequest, "BAD_REQUEST", "ont index tidak valid")
	}
	...
	data, err := h.oltManager.GetSignal(c.UserContext(), id, portIndex, ontIndex, from, to)
	if err != nil {
		return h.mapError(c, err)
	}
