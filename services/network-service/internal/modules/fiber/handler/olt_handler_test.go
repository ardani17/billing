...
type mockOLTManager struct {
	...
	getTrafficFn       func(ctx context.Context, oltID string, portIndex int, from, to time.Time) ([]domain.PONTrafficPoint, error)
	getSignalFn        func(ctx context.Context, oltID string, portIndex int, ontIndex int, from, to time.Time) ([]domain.ONTSignalPoint, error)
...
func (m *mockOLTManager) GetTraffic(ctx context.Context, oltID string, portIndex int, from, to time.Time) ([]domain.PONTrafficPoint, error) {
	if m.getTrafficFn != nil {
		return m.getTrafficFn(ctx, oltID, portIndex, from, to)
	}
	return nil, nil
}
func (m *mockOLTManager) GetSignal(ctx context.Context, oltID string, portIndex int, ontIndex int, from, to time.Time) ([]domain.ONTSignalPoint, error) {
	if m.getSignalFn != nil {
		return m.getSignalFn(ctx, oltID, portIndex, ontIndex, from, to)
	}
	return nil, nil
}
...
func setupOLTTestApp(oltMgr domain.OLTManager, alarmMgr domain.AlarmManager, odpMgr domain.ODPManager) *fiber.App {
	...
	devices.Get("/:id/pon-ports/:port/onts/:ont/signal", oltHandler.GetSignal)
...
// =============================================================================
// Tes OLT CreateOLT - POST /api/v1/olt/devices
// =============================================================================
