...
// =============================================================================
// Tes: PollAlarms menyimpan alarm dan terbitkan event
// =============================================================================
...
// =============================================================================
// Tes: PollAlarms OLT tidak ditemukan
// =============================================================================
...
// =============================================================================
// Tes: GetAlarms mendelegasikan ke repo
// =============================================================================
...
// =============================================================================
// Tes: PurgeOldAlarms dengan threshold 90 hari
// =============================================================================
...
func TestAlarmManager_FindOLTByTrapSourceMappedAndUnknown(t *testing.T) {
	am, _, _ := newTestAlarmManager(&amAdapter{})
	ctx := context.Background()

	olt, err := am.findOLTByTrapSource(ctx, "192.168.1.100")
	if err != nil {
		t.Fatalf("findOLTByTrapSource mapped gagal: %v", err)
	}
	if olt.ID != "olt-001" {
		t.Fatalf("mapped OLT ID = %q, want olt-001", olt.ID)
	}

	if _, err := am.findOLTByTrapSource(ctx, "192.168.1.250"); err != domain.ErrOLTNotFound {
		t.Fatalf("unknown trap source err = %v, want ErrOLTNotFound", err)
	}
}

func TestAlarmManager_HasActiveTrapAlarmDedupesFingerprint(t *testing.T) {
	am, alarmRepo, _ := newTestAlarmManager(&amAdapter{})
	port := 1
	ont := 7
	alarmRepo.listResult = &domain.AlarmListResult{
		Data: []*domain.OLTAlarmRecord{{
			ID:           "alarm-001",
	...
	if !am.hasActiveTrapAlarm(context.Background(), "olt-001", &domain.OLTAlarm{
		AlarmType:    domain.AlarmTypeONTLOS,
		PONPortIndex: &port,
		ONTIndex:     &ont,
	}) {
		t.Fatal("expected active trap alarm duplicate")
	}
}

func TestAlarmManager_ClearMatchingTrapAlarm(t *testing.T) {
	am, alarmRepo, _ := newTestAlarmManager(&amAdapter{})
	port := 1
	alarmRepo.listResult = &domain.AlarmListResult{
		Data: []*domain.OLTAlarmRecord{{
			ID:           "alarm-001",
			AlarmType:    domain.AlarmTypePONPortDown,
			PONPortIndex: &port,
			Status:       domain.AlarmStatusActive,
		}},
	}

	am.clearMatchingTrapAlarm(context.Background(), &domain.OLT{
		ID: "olt-001", TenantID: "tenant-001", Name: "OLT-Test",
	}, &domain.OLTAlarm{
		AlarmType:    domain.AlarmTypePONPortDown,
		Severity:     domain.SeverityClear,
		PONPortIndex: &port,
	})

	alarmRepo.mu.Lock()
	defer alarmRepo.mu.Unlock()
	if len(alarmRepo.clearedIDs) != 1 || alarmRepo.clearedIDs[0] != "alarm-001" {
		t.Fatalf("cleared IDs = %v, want [alarm-001]", alarmRepo.clearedIDs)
	}
}
