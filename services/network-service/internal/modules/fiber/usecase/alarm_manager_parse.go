...
func parseTrapPDU(pdus []gosnmp.SnmpPDU, sourceIP string) *domain.OLTAlarm {
	...
	for _, pdu := range pdus {
		...
		clearEvent := containsAny(val, "clear", "cleared", "restore", "restored", "recover", "recovered", "normal")

		switch {
		case strings.Contains(oid, "1.3.6.1.4.1") && containsAny(val, "los", "loss of signal") && clearEvent:
			alarm.AlarmType = domain.AlarmTypeONTLOS
			alarm.Severity = domain.SeverityClear
			alarm.Message = "ONT Loss of Signal cleared: " + val
