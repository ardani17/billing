// File ini berisi implementasi Buat untuk OLT Manager
...
// Buat membuat OLT baru, encrypt credentials, simpan ke DB,
// test SNMP (best-effort), auto-detect brand/model/firmware, dan kembalikan OLTResponse.
func (m *oltManager) Create(ctx context.Context, tenantID string, req domain.CreateOLTRequest) (*domain.OLTResponse, error) {
	...
	probeWarning := m.tryAutoDetect(ctx, created)

	// Tambahkan ke pemeriksa kesehatan jika tersedia
	if m.healthChecker != nil {
		m.healthChecker.AddOLT(created)
	}

	resp := oltToResponse(created)
...
func (m *oltManager) tryAutoDetect(ctx context.Context, olt *domain.OLT) string {
	snmpCfg, err := m.buildSNMPConfig(olt)
	if err != nil {
		log.Warn().Err(err).Str("olt_id", olt.ID).Msg("gagal build SNMP config untuk auto-detect")
		return "Auto-detect SNMP gagal: konfigurasi SNMP tidak valid"
	}
	...
	if err != nil {
		...
		if adapterErr != nil {
			...
			return "Auto-detect SNMP gagal: OLT disimpan sebagai offline"
		}
	...
	// Perbarui OLT dengan info yang terdeteksi
	...
	if updateErr != nil {
		...
		return "Auto-detect berhasil tetapi update profil OLT gagal"
	}
	*olt = *updated
	return ""
}
