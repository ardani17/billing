...
// Alur: validasi -> resolve router & pool -> build command -> execute -> save DB -> terbitkan event.
...
func (m *pppoeManager) HandleCustomerActivated(ctx context.Context, payload domain.CustomerActivatedPayload) error {
	...
	// Bangun parameter PPPoE secret untuk router
	...
	// Bangun dan execute perintah CreateSecret di router
	...
	if saveErr != nil {
		...
		// Tetap terbitkan event meskipun save gagal
	}

	// Terbitkan command_result event
	...
	// Kembalikan error jika eksekusi di router gagal (untuk retry oleh worker)
	if execErr != nil {
