...
func (m *staticIPManager) DeleteAssignment(ctx context.Context, routerID, assignmentID string, req domain.DeleteStaticIPAssignmentRequest) error {
	...
	if listID, _ := findStaticAddressListID(ctx, adapter, item); listID != "" {
		cmd, args := cmdBuilder.RemoveAddressList(listID)
		if _, err := adapter.Execute(ctx, cmd, args); err != nil {
			m.audit(ctx, item, "static_ip.delete", cmd, "failed", err)
			return err
		}
	}
	if item.QueueName != "" {
		_ = removeStaticQueue(ctx, adapter, cmdBuilder, item)
	}
...
func (m *staticIPManager) syncAssignment(ctx context.Context, item *domain.StaticIPAssignment) error {
	...
	cmdBuilder, err := commandBuilderForRouter(ctx, m.routerRepo, item.RouterID)
	if err != nil {
		return err
	}
	...
	if item.QueueName != "" && item.RateLimit != "" {
		if err := upsertStaticQueue(ctx, adapter, cmdBuilder, item); err != nil {
			return err
		}
	}
	now := time.Now()
...
func upsertStaticQueue(ctx context.Context, adapter domain.RouterOSAdapter, cmdBuilder domain.CommandBuilder, item *domain.StaticIPAssignment) error {
	...
	for _, row := range rows {
		if row["name"] == item.QueueName || strings.HasPrefix(row["comment"], staticIPCommentPrefix+item.ID) {
			...
			cmd, args := cmdBuilder.SetSimpleQueue(item.QueueName, params)
			_, err := adapter.Execute(ctx, cmd, args)
			return err
		}
	}
	cmd, args := cmdBuilder.CreateSimpleQueue(domain.SimpleQueueParams{
		Name:     item.QueueName,
		Target:   item.IPAddress + "/32",
		MaxLimit: item.RateLimit,
		Comment:  staticIPComment(item),
	})
	_, err = adapter.Execute(ctx, cmd, args)
	return err
...
func removeStaticQueue(ctx context.Context, adapter domain.RouterOSAdapter, cmdBuilder domain.CommandBuilder, item *domain.StaticIPAssignment) error {
	rows, err := adapter.Execute(ctx, "/queue/simple/print", map[string]string{"=.proplist": ".id,name,comment"})
	if err != nil {
		return err
	}
	for _, row := range rows {
		if row["name"] == item.QueueName || strings.HasPrefix(row["comment"], staticIPCommentPrefix+item.ID) {
			cmd, args := cmdBuilder.RemoveSimpleQueue(row[".id"])
			_, err := adapter.Execute(ctx, cmd, args)
			return err
		}
	}
