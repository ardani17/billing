...
// Alur: get router & pool -> encrypt password -> build command -> execute -> save DB.
...
func (m *pppoeManager) CreateUser(ctx context.Context, routerID string, req domain.CreatePPPoEUserRequest) (*domain.PPPoEUser, error) {
	...
	// Bangun comment untuk tracking ownership di router
	comment := domain.BuildComment(req.CustomerID, tenantID)

	// Bangun parameter PPPoE secret
...
// Alur: get user -> get router & pool -> /ppp/secret/atur -> perbarui DB.
...
// DeleteUser menghapus PPPoE user dari router dan hapus lunak dari database.
// Alur: get user -> get router & pool -> disconnect -> remove secret -> remove queue -> remove firewall -> hapus lunak DB.
