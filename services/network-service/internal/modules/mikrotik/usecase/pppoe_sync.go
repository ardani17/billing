...
// Alur: ambil secrets dari router -> ambil users dari DB -> bandingkan -> auto-fix.
// Database adalah sumber of truth untuk user yang dikelola ISPBoss.
func (m *pppoeManager) SyncRouter(ctx context.Context, routerID string) (*domain.SyncResult, error) {
	...
	// Step 1: Ambil router dan koneksi pool dengan PriorityLow (pemantauan)
	...
	for _, dbUser := range dbUsers {
		...
		// User aktif di DB tapi tidak ada di router -> missing, auto-buat
...
// parseDisabledField mengurai field "disabled" dari router respons.
