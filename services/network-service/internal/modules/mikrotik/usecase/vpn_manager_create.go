...
// Langkah: validasi -> cek router -> get/buat subnet -> alokasi IP -> buat key -> encrypt -> simpan -> terbitkan event.
func (m *vpnManager) CreateTunnel(ctx context.Context, tenantID string, req domain.CreateVPNTunnelRequest) (*domain.VPNTunnelResponse, error) {
	...
	// Buat key/credential sesuai protokol
	...
	// Terbitkan event vpn_tunnel_created (best-effort, log error jika gagal)
...
func (m *vpnManager) validateRouterVersion(ctx context.Context, routerID, protocol string) error {
	...
	if protocol == string(domain.ProtocolWireGuard) && !domain.CapabilitiesForRouterOS(router.RouterOSVersion).SupportsWireGuard {
...
func (m *vpnManager) generateTunnelCredentials(
	...
	// Bandwidth cap bawaan - akan diupdate berdasarkan tier tenant oleh billing service
	defaultBwCap := 50 // bawaan Growth tier (Mbps)
