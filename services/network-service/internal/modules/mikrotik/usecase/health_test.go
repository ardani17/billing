...
// mockEventPublisher merekam panggilan terbitkan event.
...
func TestProperty5_SuccessfulHealthCheckResetsFailureState(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// System resource dari health cek yang berhasil
		...
		if repo.updatedRouter == nil {
			t.Fatal("metadata router harus di-refresh setelah health check sukses")
		}
		if repo.updatedRouter.RouterOSVersion != sysRes.Version {
			t.Fatalf("router_os_version = %q, want %q", repo.updatedRouter.RouterOSVersion, sysRes.Version)
		}
		if repo.updatedRouter.BoardName != sysRes.BoardName {
			t.Fatalf("board_name = %q, want %q", repo.updatedRouter.BoardName, sysRes.BoardName)
		}
		if repo.updatedRouter.TotalRAMMB != 256 {
			t.Fatalf("total_ram_mb = %d, want 256", repo.updatedRouter.TotalRAMMB)
		}
...
// **Memvalidasi: Kebutuhan 6.3, 6.4**
//
// Untuk router dengan failure_count N (0 <= N < 3),
// setelah health cek gagal:
...
func TestProperty6_FailedHealthCheckIncrementsFailureCount(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// Buat failure_count 0-2 (belum mencapai threshold)
		...
		if expectedCount >= failureThreshold {
			// Status harus di-atur ke offline (kecuali sudah offline)
			if status != domain.StatusOffline {
				if repo.lastHealthCheck.Status == nil {
...
//
// pemeriksa kesehatan HARUS mendeteksi reboot dan terbitkan event
