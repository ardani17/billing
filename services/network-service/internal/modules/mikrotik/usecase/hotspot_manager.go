...
func (m *hotspotManager) CreateUser(ctx context.Context, routerID string, req domain.CreateHotspotUserRequest) (*domain.HotspotUser, error) {
	...
	cmd, args := cmdBuilder.AddHotspotUser(params)
	if _, err := adapter.Execute(ctx, cmd, args); err != nil {
		m.audit(ctx, routerID, "hotspot.user.create", cmd, "failed", err)
		return nil, err
	}
	m.audit(ctx, routerID, "hotspot.user.create", cmd, "success", nil)
...
func (m *hotspotManager) UpdateUser(ctx context.Context, routerID, userID string, req domain.UpdateHotspotUserRequest) (*domain.HotspotUser, error) {
	...
	cmd, args := cmdBuilder.SetHotspotUser(params)
	if _, err := adapter.Execute(ctx, cmd, args); err != nil {
		m.audit(ctx, routerID, "hotspot.user.update", cmd, "failed", err)
		return nil, err
	}
	m.audit(ctx, routerID, "hotspot.user.update", cmd, "success", nil)
	return findHotspotUserByID(ctx, adapter, userID)
}
...
func (m *hotspotManager) DeleteUser(ctx context.Context, routerID, userID string) error {
	...
	cmd, args := cmdBuilder.RemoveHotspotUser(userID)
	if _, err := adapter.Execute(ctx, cmd, args); err != nil {
		m.audit(ctx, routerID, "hotspot.user.delete", cmd, "failed", err)
		return err
	}
	m.audit(ctx, routerID, "hotspot.user.delete", cmd, "success", nil)
	return nil
}
