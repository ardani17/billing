...
// Digunakan oleh usecase untuk membuat adapter sementara (test connection, buat).
...
// Buat membuat router baru dan menyimpan credential terenkripsi.
// Tes koneksi bersifat opt-in lewat TestOnCreate agar router tidak menerima
...
func (uc *routerUsecase) Create(ctx context.Context, tenantID string, req domain.CreateRouterRequest) (*domain.RouterResponse, error) {
	...
	// Tes koneksi dan auto-detect info (best-effort)
	warning := ""
	...
	if connErr != nil {
		...
		// Perbarui router dengan info dari system resource
		...
		// Terbitkan event router online (best-effort)
...
// Perbarui memperbarui data router. Encrypt password jika berubah.
// Perbarui pool jika host/port/credentials berubah.
...
// Hapus melakukan hapus lunak router dan menutup pool koneksi.
...
// Digunakan oleh Buat dan TestConnection.
