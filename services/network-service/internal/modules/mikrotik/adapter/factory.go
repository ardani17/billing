...
// Bawaan ke MockAdapter jika mode tidak dikenali (aman untuk development).
package adapter
...
// Mode "mock" atau mode lainnya mengembalikan MockAdapter (bawaan aman untuk development).
func NewAdapter(mode string) RouterOSAdapter {
	switch mode {
	case "live":
		return NewLiveAdapter()
	default:
		// Bawaan ke MockAdapter untuk keamanan saat development.
